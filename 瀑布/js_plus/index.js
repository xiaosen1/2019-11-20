...
findIndex(callback)
    ...
    问题：
        当滚轮的时候，window.onscroll触发的次数非常多如果是触底去请求服务器，那么会在同一时间请求若干次
        这样大大增加了服务器的压力，也降低了用户的体验

        所以解决这个问题的方案有2个
        第一个：（停下来才触发）  防抖
                事件出发的频率很高，但是只要停下来再加载
        第二个：（降频率）  节流
                每隔固定时间再加载图片，比如每隔200s
*/
function minEle(lis) {
    // ary就是每隔li的被内容撑开的高度
    let ary = [...lis].map(ele.scrollHeight)
    // 找到li高度最短的值
    let min = Math.min(...ary)
    // 根据最短的值，对应的索引，有了这个索引就能应对li
